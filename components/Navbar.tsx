...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* ── NAV ── */}
      <nav className={`fixed top-0 left-0 right-0 z-[9999] transition-all duration-300 ${scrolled ? "bg-[#0D0D0D]/95 backdrop-blur-xl shadow-[0_1px_0_rgba(0,255,100,0.15),0_8px_32px_rgba(0,0,0,0.5)] py-2" : "bg-transparent py-4"}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-8">
          ...
          <button type="button" onClick={() => handleClick("#home")} className="flex items-center flex-shrink-0 group" aria-label="Home">
            <Image src="/logo.png" alt="MarkBrand Group Nigeria" width={140} height={48} className="h-11 w-auto object-contain transition-opacity duration-200 group-hover:opacity-75" priority />
          </button>
      ...
      >
        ...
        <div className="flex flex-col flex-1">
          ...
          <div className="mt-auto pt-8">
            <button type="button" onClick={() => handleClick("#contact")} className="block w-full text-center py-3.5 text-xs font-semibold tracking-[0.14em] uppercase text-[#0D0D0D] bg-[#00ff64] rounded-sm hover:bg-[#00cc50] transition-colors duration-200">
              Let&apos;s Talk
            </button>
          </div>
